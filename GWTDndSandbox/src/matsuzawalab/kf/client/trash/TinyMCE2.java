 ...
 */
public class TinyMCE2 extends SimplePanel {

	private HTML ta;
	private String id;

	public TinyMCE2(String initialContent) {
		...
		this.addAttachHandler(new AttachEvent.Handler() {
			...
			public void onAttachOrDetach(AttachEvent event) {
				if (!event.isAttached()) {// detach
					...
					init(id);
					NodeList<Element> elements = getElement()
							.getElementsByTagName("a");
					int len = elements.getLength();
					for (int i = 0; i < len; i++) {
						Element elem = elements.getItem(i);
						final String url = ((AnchorElement) elem).getHref();
						DOM.sinkEvents(elem, Event.ONMOUSEDOWN);
				...
				.init({
					...
					setup : function(ed) {
						...
						ed.on('show', function(e) {
							$wnd.console.log("show" + text);
						});
						ed.on('hide', function(e) {
							$wnd.console.log("hide" + text);
						});
						ed
								.on(
										'drop',
...
//
// // Safari doesn't support custom dataTransfer items so we can only use URL
// and Text
// if (selectionHtml.length > 0) {
// e.dataTransfer.setData('URL', 'data:text/mce-internal,' +
...
//
// if (!internalContent || internalContent.indexOf(urlPrefix) == -1 ||
