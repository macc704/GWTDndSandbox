...
public final class WindowExample extends Example {
	...
	public WindowExample(DemoDragHandler demoDragHandler) {
		...
		windowController.getPickupDragController().addDragHandler(
				demoDragHandler);
		...
		HTML header2 = new HTML(
				"A draggable &amp; resizable <code>IFRAME</code>");
		Frame iframe2 = getIframe(IFRAME_URL);
		WindowPanel windowPanel2 = new WindowPanel(windowController, header2,
				iframe2, false);
		...
		WindowPanel windowPanel3 = new WindowPanel(windowController, header3,
				image3, false);
		boundaryPanel.add(windowPanel3, 70, 30);

		// create the fourth panel
		HTML header4 = new HTML(
				"A draggable &amp; resizable <code>TEXTAREA</code>");
		TextArea textArea4 = new TextArea();
		textArea4.addStyleName("demo-resize-html");
		textArea4.setText(makeText());
		WindowPanel windowPanel4 = new WindowPanel(windowController, header4,
				textArea4, false);
	...
	public Class<?>[] getInvolvedClasses() {
		return new Class[] { WindowExample.class, WindowController.class,
				WindowPanel.class, ResizeDragController.class, };
	}

	private Frame getIframe(String url) {
		Frame iframe = new Frame(url);
		//DOM.setElementAttribute(iframe.getElement(), "frameBorder", "no");
		iframe.getElement().setAttribute("frameBorder", "no");
